...
class Manager:
    ...
    def __init__(self):
        ...
        self.sql = RDS()
        self.tmz = pytz.timezone(os.getenv("WEB_TIMEZONE"))
        self.spatialcols = ['timestamp', 'longitude', 'latitude']
    ...
    def city_table(self, city_id):

        qry = f"SELECT city, state, max_date, department \
                FROM Cities WHERE id='{city_id}'"
        cty = self.sql.unique(qry)
        ...
        cty.update({"table": nme})

        return cty

    @staticmethod
    def record_to_key(record, keys, intervals):

        dte = parser.parse(f"{record.get('date')} {record.get('time', '00:00')}")

        if dte < intervals[0]:
            return None
        elif dte > intervals[-1]:
            return None
        else:
            dif = [r - dte for r in intervals]
            idx = len([r for r in dif if r.total_seconds() < 0]) - 1
            return keys[idx]
    ...
    @staticmethod
    def build_geojson(records, info_col):
    ...
    @staticmethod
    def list_dates(start, end, query=False):

        if end == None: end = start
        u,v = parser.parse(start), parser.parse(end)
        dlt = (v - u).days
        lst = [(u + timedelta(days=i)).strftime('%Y-%m-%d') for i in range(dlt + 1)]

        if query:
            lst = [r"'{}'".format(e) for e in lst]
            lst = f"({', '.join(lst)})"

        return lst

    @staticmethod
    def dates_to_timestamps(self, start, end):

        if end == None: end = start
        u, v = parser.parse(start), parser.parse(end)
        u = datetime.datetime.combine(u, datetime.time.min).timestamp()
        v = datetime.datetime.combine(v, datetime.time.max).timestamp()

        return int(u), int(v)

    def valid_dates(self, table):

        qry = f"SELECT min(date) as minDate, max(date) as maxDate FROM {table} \
                WHERE date IS NOT NULL AND date<>'None'"
        res = self.sql.get_unique(qry)
        if res is None:
            u, v = None, None
        else:
            u, v = res.get('minDate', None), res.get('maxDate', None)

        return {'minDate': u, 'maxDate': v}
